    ...
    <div className="px-[5%] py-10">
      <div className="grid gap-10 grid-cols-2">
        ...
        <div className="border p-4">
          ...
          {isInCart && (
            <Link to="/user/cart" className="px-4 py-2 rounded ml-2 bg-gray-100">
            Go to Cart</Link>
          )}
          <button type="button" onClick={() => toggleWishlist(product)} className="px-2">
            {isInWishlist(product.id) ? "❤️" : "🤍"}
          </button>
          {isInWishlist(product.id) && (
            <Link to="/wishlist" className="px-4 py-2 rounded ml-2 bg-gray-100">
            Go to Wishlist</Link>
          )}
        </div>
      </div>
